...
class RNN:
    def __init__(self, hidden_size, vocab_size, seq_length, learning_rate):
        ...
        # model parameters
        self.U = np.random.uniform(0,1, (hidden_size, vocab_size))
        self.U=np.around(self.U,decimals=2)
        self.ctr=0
        print("\nInitlal weights Wxh\n",self.U)
        self.V = np.random.rand(vocab_size, hidden_size)
        self.V=np.around(self.V,decimals=2)
        print("\nInitlal weights Why\n",self.V)
        self.W = np.random.rand(hidden_size,hidden_size)
        self.W=np.around(self.W,decimals=2)
        print("\nInitlal weights Whh\n",self.W)
        self.b = np.zeros((hidden_size, 1)) # bias for hidden layer
        print("\nInitial bias for hidden layer b is\n",self.b)
        self.c = np.zeros((vocab_size, 1)) # bias for output
        print("\nInitial bias for output c is\n",self.c)
    ...
    def train(self, data_reader):
            ...
            while (smooth_loss > threshold):
                ...
                iter_num += 1
            print("\nFinal Iteration number ",iter_num)
            print("\nFinal weights Wxh is\n ",self.U)
            print("\nFinal weights Why is\n",self.W)
            print("\nFinal weights Whh is\n",self.V)
            print("\nFinal bias for hidden layer b is\n",self.b)
    ...
    def predict(self, data_reader, start, n):
        ...
        for t in range(n):
            print("the initial weight wxh",self.U)
            print("the final weight matrix whh",self.W)
            h = np.tanh(np.dot(self.U, x) + np.dot(self.W, h) + self.b)
            y = np.dot(self.V, h) + self.c
            p = np.exp(y)/np.sum(np.exp(y))
            ix = np.random.choice(range(self.vocab_size), p = p.ravel())
...
predicted = rnn.predict(data_reader, 'p', 5)
